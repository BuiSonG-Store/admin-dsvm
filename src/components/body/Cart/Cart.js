    ...
    try {
      if (gmail) {
        const res = await axios.get(
            process.env.REACT_APP_HOST +`/api/Accounts/gmail?gmail=${gmail}`
        );
        setUser(res.data);
      }
    } catch (e) {
...
const [checkValidity, setCheckValidity] = useState(true);
  ...
  const handleSubmitForm = async () => {
    ...
    if (isValid) {
        try {
            const billRes = await axios.post(process.env.REACT_APP_HOST +'/api/Bills',billData);

            await getCartItems().forEach( (item, index) => {
                 axios.post(process.env.REACT_APP_HOST +'/api/Carts',{
                        amount: item.amount,
                        productId: item.id,
                                  ...
                                  <tr>
                                    ...
                                    <td className="product-subtotal">
                                      <span className="subtotal-amount">
                                        {fCurrency(item.price * amount[index])}
                                      </span>
                                      <a
                    ...
                    <div className="col-lg-6">
                     ...
                     <>
                      <div className="cart-totals">
                        ...
                        <ul>
                          <li>
                            Tổng phụ <span>{fCurrency(subTotal)}</span>
                          </li>
                          <li>
                            Phí ship <span>chưa xác định</span>
                          </li>
                          <li>
                            Tổng tiền{" "}
                            <span>
                              <b>{fCurrency(subTotal)} + phí ship</b>
                            </span>
                          </li>
                        </ul>
                        <button
